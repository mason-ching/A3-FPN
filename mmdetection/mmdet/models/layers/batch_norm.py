...
class GRN(nn.Module):
    ...
    def forward(self, x: Tensor, data_format='channel_first'):
        """Forward method.
        ...
        if data_format == 'channel_last':
            gx = torch.norm(input=x, p=2.0, dim=[1, 2], keepdim=True)
            nx = gx / (gx.mean(dim=-1, keepdim=True) + self.eps)
            x = self.gamma * (x * nx) + self.beta + x
        elif data_format == 'channel_first':
            gx = torch.norm(input=x, p=2.0, dim=[2, 3], keepdim=True)
            nx = gx / (gx.mean(dim=1, keepdim=True) + self.eps)
...
def get_norm(norm, out_channels, **kwargs) -> nn.Module or None:
    """
    Args:
        out_channels:
        norm (str or callable): either one of BN, SyncBN, FrozenBN, GN, GRN;
    ...
    if isinstance(norm, str):
        ...
        norm = {
            ...
            "LN": lambda channels: LayerNorm(channels, **kwargs),
            'GRN': GRN
        }[norm]
...
class LayerNorm(nn.Module):
    r"""This a LayerNorm variant, popularized by Transformers, that performs point-wise mean and
    variance normalization over the channel dimension.
