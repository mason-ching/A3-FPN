...
@SEM_SEG_HEADS_REGISTRY.register()
class A3FPNMSDeformAttnPixelDecoder(nn.Module):
    @configurable
    def __init__(
            self,
            input_shape: Dict[str, ShapeSpec],
    ...
    def from_config(cls, cfg, input_shape: Dict[str, ShapeSpec]):
        ...
        a3fpn_in_channels = [cfg.MODEL.SEM_SEG_HEAD.CONVS_DIM] * \
                            len(cfg.MODEL.SEM_SEG_HEAD.DEFORMABLE_TRANSFORMER_ENCODER_IN_FEATURES)
        for i in cfg.MODEL.SEM_SEG_HEAD.IN_FEATURES[::-1]:
            if i not in cfg.MODEL.SEM_SEG_HEAD.DEFORMABLE_TRANSFORMER_ENCODER_IN_FEATURES:
                a3fpn_in_channels.insert(0, *[v.channels for k, v in input_shape.items() if i == k])
